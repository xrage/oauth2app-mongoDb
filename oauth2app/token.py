...
class TokenGenerator(object):
    ...
    * *refreshable:* Boolean value indicating whether issued tokens are
    ...
    def _validate_authorization_code(self):
        ...
        self.scope = set([x for x in self.code.scope])
    ...
    def _validate_refresh_token(self):
        ...
        if self.scope is not None:
            access_ranges = set([x for x in self.access_token.scope])
    ...
    def _get_authorization_code_token(self):
        ...
        access_ranges = self.code.scope if self.scope else []
        access_token.scope = access_ranges
        print access_token.mac_key
        print access_token.token
        print access_token.scope
        print access_token.refresh_token
        print access_token.refreshable
        access_token.save()
        self.code.delete()
        return access_token
    ...
    def _get_password_token(self):
        ...
        if self.authentication_method == MAC:
            access_token.mac_key = KeyGenerator(MAC_KEY_LENGTH)()
        new_scope = []
        for sc in AccessRange.objects.filter(key__in=self.scope):
            new_scope.append(sc.key)
        access_token.scope = new_scope
        access_token.save()
        return access_token
    ...
    def _get_refresh_token(self):
        ...
        for sc in AccessRange.objects.filter(key__in=self.scope):
            new_scope.append(sc.key)
        if self.scope:
            self.access_token.scope = new_scope
        elif len(self.access_token.scope)>0:
